...
fn process_init_reserve(
    ...
    if config.liquidation_threshold <= config.loan_to_value_ratio || config.liquidation_threshold > 100 {
    ...
    if reserve_info.owner != program_id {
        println!("Reserve provided is not owned by the lending program");
        msg!("Reserve provided is not owned by the lending program");
        return Err(LendingError::InvalidAccountOwner.into());
    }

    if reserve_liquidity_supply_info.key == source_liquidity_info.key {
        println!("Reserve liquidity supply cannot be used as the source liquidity provided");
        msg!("Reserve liquidity supply cannot be used as the source liquidity provided");
        return Err(LendingError::InvalidAccountInput.into());
    }
    let lending_market = LendingMarket::unpack(&lending_market_info.data.borrow())?;
    if lending_market_info.owner != program_id {
        println!("Lending market provided is not owned by the lending program");
        msg!("Lending market provided is not owned by the lending program");
        return Err(LendingError::InvalidAccountOwner.into());
    }
    ...
    if &lending_market.owner != lending_market_owner_info.key {
        println!("Lending market owner does not match the lending market owner provided");
        msg!("Lending market owner does not match the lending market owner provided");
        return Err(LendingError::InvalidMarketOwner.into());
    }
    if !lending_market_owner_info.is_signer {
        println!("Lending market owner provided must be a signer");
        msg!("Lending market owner provided must be a signer");
        return Err(LendingError::InvalidSigner.into());
    }

    if &lending_market.oracle_program_id != pyth_product_info.owner {
        println!("Pyth product account provided is not owned by the lending market oracle program");
    ...
    if &lending_market.oracle_program_id != pyth_price_info.owner {
        println!("Pyth price account provided is not owned by the lending market oracle program");
    ...
    if pyth_product.magic != pyth::MAGIC {
        println!("Pyth product account provided is not a valid Pyth account");
    ...
    if pyth_product.atype != pyth::AccountType::Product as u32 {
        println!("Pyth product account provided is not a valid Pyth product account");
    ...
    if &pyth_product.px_acc.val != pyth_price_pubkey_bytes {
        println!("Pyth product price account does not match the Pyth price provided");
    ...
    if lending_market.quote_currency != quote_currency {
        println!("Lending market quote currency does not match the oracle quote currency");
    ...
    if &lending_market_authority_pubkey != lending_market_authority_info.key {
        println!("Derived lending market authority does not match the lending market authority provided");
    ...
    if reserve_liquidity_mint_info.owner != token_program_id.key {
        println!("Reserve liquidity mint is not owned by the token program provided");
...
fn assert_rent_exempt(rent: &Rent, account_info: &AccountInfo) -> ProgramResult {
    ...
    println!("{} not rent exempt", account_info.key);

    if !rent.is_exempt(account_info.lamports(), account_info.data_len()) {
        println!("{}", &rent.minimum_balance(account_info.data_len()).to_string());
